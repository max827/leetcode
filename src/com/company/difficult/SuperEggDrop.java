...
public class SuperEggDrop {
    ...
    public int superEggDrop(int K, int N) {
        ...
        //如果鸡蛋个数为0，则无法测试，相当于测试次数为0
        dp[1][0] = 0;
        //如果楼层个数只有1层，则所有测试次数全部为1
        for (int j = 1; j <= K; j++) {
            dp[1][j] = 1;
        }
        //假设鸡蛋只有0个过一个的情况下的测试次数
        for (int i = 0; i <= N; i++) {
            dp[i][0] = 0;
            dp[i][1] = i;
        }
        ...
        // 开始递推
        //前面的初始化将楼层数在两层以下和鸡蛋数在两个以下的情况都考虑进去并设置了默认值所以循环直接从2开始
        for (int i = 2; i <= N; i++) {
            for (int j = 2; j <= K; j++) {
                // 在区间 [1, i] 里确定一个最优值
                int left = 1;
